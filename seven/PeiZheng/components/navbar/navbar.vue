	<template>
		<view class="nav">
			...
			<view v-if="isHome" class="headNav" :style="'height' + navHeight + 'rpx;line-height:' + navHeight + 'rpx;padding-left: 20rpx;'">
				...
				<view style="flex: 1">
					<navigator url="../../pages/seach/index" 
					:style="
					'height:' + 
					menu.height * 2 + 
					'rpx;line-height:' + 
			...
			<view v-else class="navbar" :style = "'height:' + navHeight + 'rpx;' + containerStyle">
				<view class="back-icon" @click="backOrHome">
					<image v-if="pages > 1" src="../../static/ic_back.png"></image>
					<image v-else src="../../static/ic_home.png"></image>
				</view>
	...
	<script setup>
		...
		const emit = defineEmits(['onNavBarAttached'])
		onBeforeMount(() => {
			setNavSize();
			setStyle();
			emit('navBarHeight',{
