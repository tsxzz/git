...
<script setup>
	...
	const toMap = () => {
		...
		uni.navigateTo({ url: 'plugin://routePlan/index?key=' + key + '&referer=' + referer + '&endPoint=' + endPoint + '&navigation=1' })
	 }
	
	const bMapTransQQMap = (lng, lat) => {
    let x_pi = (3.141592653589793 * 3000) / 180; // 计算π的值，用于后续计算
    let x = lng - 0.0065; // 对经度进行偏移
    let y = lat - 0.006; // 对纬度进行偏移
    let z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * x_pi); // 计算z值，涉及到x和y的平方和以及正弦函数
    let theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * x_pi); // 计算theta值，涉及到y和x的反正切函数以及余弦函数
    let lngs = z * Math.cos(theta); // 计算转换后的经度
    let lats = z * Math.sin(theta); // 计算转换后的纬度
    return {
        lng: lngs, // 返回转换后的经度
        lat: lats  // 返回转换后的纬度
		};
	...
	const toService = (e) => {
		...
		uni.navigateTo({ url: '../service/index?hid=' + hospital.value.id + '&svid=' + e.currentTarget.dataset.svid})
	}
