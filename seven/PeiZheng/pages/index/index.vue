	...
	<view>
		...
		<view style="width: 100%;">
			<view class="weui-cell" style="background:#fff9eb;">
				<view class="weui-cell-hd">
					<image src="/static/ic_myapp.png" style="display:block;width:40rpx;height:40rpx;margin-right:14rpx;"></image>
				</view>
				<view class="weui-cell-bd">
					<text style="color:#be9719;font-size:26rpx;">点击右上角"添加到我的小程序"，方便下次找到！</text>
				</view>
				<view class="weui-cell-ft">
					<image src="/static/modal_closer.png" style="display:block;width:15px;height:15px;"></image>
				</view>
			</view>
		</view>
		...
		<view v-if="navs && navs.length > 0" class="nav-list">
				...
				<view class="nav-item" @click="onNavTop" :data-index="index">
					...
					<view class="nav-text" :style="'color:' + (item.tcolor ? iitem.tcolor :'')">{{ item.title }}</view>
				</view>
			</block>
		</view>
		<view>
			<view class="weui-cells hosp-list">
				<!--医院列表-->
				<view class="weui-cell hosp-item weui-cell_access" @click="onHospital" v-for="(item) in hospitals" :key="item.id" :data-hid="item.id">
...
<script setup>
	...
	const jump = (nav,type) => {
		//判断是否为内部链接
		if(nav.stype == 1){
			uni.navigateTo({ url: nav.stype_link })
		}
	}
	//医院跳转
	const onHospital = (e) => {
		console.log(e.currentTarget.dataset.hid);
		uni.navigateTo({ url: '../hospital/index?hid=' +  e.currentTarget.dataset.hid})
	}
</script>
